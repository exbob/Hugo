...
通常情况下，我们会使用一个硬件原厂发布的 BPS 源码编译系统镜像，BSP 源码包含了 Yocto 项目主仓库的 poky 包，还有原厂加入的扩展包，当完成了第一次系统构建后，工程目录下的文件结构大致如下：
...
系统镜像也是通过 recipe 文件编译出来的，我们可以在源码目录下执行如下命令查找：

``` bash
$ ls meta*/recipes*/images/*.bb
```

比如，我们编译的 imx-image-multimedia 位于：

```
meta-freescale-distro/recipes-fsl/images/fsl-image-multimedia.bb
```

您可以自定义系统镜像以满足特定的要求，比如向系统镜像添加、删除一些指定的包。本节将介绍几种方法，并为每种方法提供指导。

### 5.5.1. 使用 local.conf

可能最简单的自定义映像的方法是通过 local.conf 配置文件来添加一个包。添加的方法是在 local.conf 文件中新增带 _append 操作符的 IMAGE_INSTALL 变量：

```
IMAGE_INSTALL_append = " strace"
```

注意语法，引号和 strace 前面的空格都是必须的，因为 _append 操作符不会添加空格。此外，如果你想避免排序问题，你必须使用 _append 代替 += 操作符。原因是，这样做是无条件地追加到变量上，避免了由于变量在图像配方和 .bbclass 文件中被设置为像?=这样的运算符而引起的排序问题。 使用 _append 可以确保操作生效。

由于仅限于本地使用，这种方法一般只允许你添加包，而不像创建自己的自定义映像那样灵活。当你用这种方式使用本地变量添加包时，你需要意识到这些变量的变化对每次构建都是有效的，因此会影响所有的镜像，这可能不是你所需要的。也可以扩展语法 _append_pn，使变量只适用于一个特定的镜像。下面是一个例子。

```
IMAGE_INSTALL_append_pn-core-image-minimal = " strace"
```

这样添加的 strace 包只对 core-image-minimal 镜像生效。

### 5.5.2. 使用 MAGE_FEATURES 和 EXTRA_IMAGE_FEATURES 

另一种自定义方法是使用 IMAGE_FEATURES 和 EXTRA_IMAGE_FEATURES 变量来启用或禁用高级特征。虽然这两个变量的功能几乎是相同的，但最好的做法是在 recipe 中使用IMAGE_FEATURES，而在您的 local.conf 文件中使用 EXTRA_IMAGE_FEATURES。比如，我们要向最终镜像添加 ssh-server-openssh 包，可以在 local.conf 中添加

```
EXTRA_IMAGE_FEATURES += “ssh-server-openssh”
```

也可以在 fsl-image-multimedia.bb 文件中添加：
...
Linux 系统在 /etc/hostname 文件中定义了 hostname ，默认情况下，Yocto 在 local.conf 文件中通过 MACHINE 变量设置这个名字。你可以使用 Append 文件或 Configuration文件改变 base-files 配方中 "hostname"变量的值来定制这个名字。在 Append 文件中可以添加：

```
hostname="myhostname"
```

在 Configuration 文件中可以添加：
...
软件通常会在源码包的某个文件中声明自己的许可证类型，比如 COPYING, LICENSE 和 README 等，查到许可证类型就可以设置 LICENSE ，对于标准许可证，使用 poky/meta/files/common-licenses/ 中的文件名，或者 meta/conf/licenses.conf 中定义的 SPDXLICENSEMAP 标志名即可，也可以使用自定义的许可证名称。然后用许可证文件的 md5 校验值设置 [LIC_FILES_CHKSUM](#var-LIC_FILES_CHKSUM) 。如果源码包里没有许可证文件，也可以用任意文件替代。
...
当你创建一个追加文件时，你必须使用与对应的配方文件相同的根名。例如，追加文件someapp_3.1.2.bbappend 必须应用到 someapp_3.1.2.bb。这意味着原始配方和追加文件的名称是针对版本号的。如果重命名相应的配方以更新到较新的版本，你也必须重命名并可能更新相应的.bbappend。在构建过程中，如果 BitBake 检测到一个 .bbappend 文件，而该文件没有对应的配方与名称相匹配，则会在启动时显示一个错误。请参阅 BB_DANGLINGAPPENDS_WARNONLY 变量，了解如何处理这个错误。
...
在 recipe 中，注意SRC_URI变量，它告诉 OpenEmbedded 构建系统在构建过程中在哪里找到文件。下面是 append 文件，名为 formfactor_0.0.bbappend ，来自meta-raspberrypi/recipes-bsp/formfactor :

```
FILESEXTRAPATHS_prepend := "${THISDIR}/${PN}:"
```

默认情况下，构建系统使用FILESPATH变量来定位文件。这个 append 文件通过设置 FILESEXTRAPATHS 变量来扩展这些位置。在 .bbappend 文件中设置这个变量是将目录添加到构建系统用来查找文件的搜索路径中的最可靠和最推荐的方法。

本例中的语句将目录扩展为包括 ${THISDIR}/${PN}，它的解析结果是在append文件所在的同一目录下的一个名为 formfactor 的目录（即meta-raspberrypi/recipes-bsp/formfactor。这意味着你必须设置好支持的目录结构，它将包含你将从层中包含的任何文件或补丁。

使用立即扩展赋值运算符 := 是很重要的，因为它引用了thisdir。尾部的冒号字符很重要，因为它确保列表中的项目保持冒号分隔。

注意，BitBake 会自动定义 THISDIR 变量，你不应该自己设置这个变量。使用"_prepend "作为FILESEXTRAPATHS的一部分，可以确保你的路径会在最终列表中的其他路径之前被搜索到。另外，并不是所有的追加文件都会添加额外的文件。很多 append 文件只是为了添加构建选项而存在（例如systemd）。对于这些情况，你的 append 文件甚至不会使用 FILESEXTRAPATHS 语句。
...
构建时，${[THISDIR](#var-THISDIR)}/${[PN](#var-PN)} 会被替换为 meta-mylayer/recipes-kernel/linux/linux-imx ，构建系统会到该路径下搜索补丁文件。
...
Yocto 的应用开发有两种方式，一种是在 yocto 项目内新建recipe ，另一种是导出 SDK ，然后使用 SDK 独立开发应用软件，第二种更方便一点。
